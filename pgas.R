...
pgas = function(M, y, prior, N, sig2.init){
...
## Conditional particle filter with ancestral sampling
# x.c:  conditional reference path
# cond: conditioning (T/F)


cpf_as = function(y, sig2, N, x.c, cond){
  L = length(y)
  phi = 0.9
  sig = sqrt(sig2)
  ...
  for(t in 1:L){
    if(t!=1){
      ...
      if(cond){
        ...
        w_as = w[,t-1]*m
        w_as = w_as/sum(w_as)
        ind[N] = sample(1:N, size = 1, prob = w_as)
      }
